...
import { ShieldAlert, AlertTriangle, ExternalLink, Check, X } from "lucide-react"
...
export function SuspiciousSubmissions() {
  const flaggedSubmissions = mockSubmissions.filter(s => s.riskScore === "Medium" || s.riskScore === "High")
          ...
          <Table>
            ...
            <TableBody>
              {flaggedSubmissions.map(sub => (
                <TableRow key={sub.id} className={`${sub.riskScore === "High" ? "bg-red-50/20 dark:bg-red-950/20" : ""} border-zinc-50 dark:border-zinc-800/50 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors`}>
                  <TableCell className="px-8 py-6">
                    <Badge variant={sub.riskScore === "High" ? "danger" : "warning"} className="flex w-max items-center gap-1.5 rounded-lg font-black uppercase tracking-tight">
                  ...
                  <TableCell className="py-6">
                    <div className="flex flex-col">
                      <span className="font-bold text-zinc-950 dark:text-zinc-50">{sub.campaignTitle}</span>
                      <a href={sub.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[10px] font-black text-brand-600 hover:text-brand-500 uppercase tracking-widest mt-1">
                  ...
                  <TableCell className="py-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-zinc-600 dark:text-zinc-400">{sub.creatorName}</span>
                    </div>
                  </TableCell>
                  <TableCell className="py-6 max-w-[200px]">
                    <span className="text-xs font-medium text-zinc-500 leading-relaxed italic">
                      {sub.riskScore === "High" ? "Engagement velocity exceeds human norms." : "Quality reported below brand benchmarks."}
                    </span>
                  </TableCell>
                  <TableCell className="px-8 py-6 text-right">
                    <div className="flex gap-2 justify-end">
                      <Button variant="outline" size="sm" className="h-10 px-4 rounded-xl text-green-600 border-green-50 hover:bg-green-50 font-bold active:scale-95 transition-all" title="Verified Safe">
                        <Check className="h-4 w-4 mr-2" /> False Alarm
                      </Button>
                      <Button variant="outline" size="sm" className="h-10 px-4 rounded-xl text-red-600 border-red-50 hover:bg-red-50 font-bold active:scale-95 transition-all" title="Revoke & Restrict">
